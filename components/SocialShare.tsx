...
import { X, Link as LinkIcon, Smartphone, Facebook, Linkedin, Instagram, Copy, Twitter, Check, Send } from 'lucide-react';
...
const SocialShare: React.FC<SocialShareProps> = ({ url, title, onClose, subtitle }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 text-gray-900">
        <div className="bg-brand-black p-4 flex justify-between items-center text-white">
          <div>
            <h3 className="font-bold text-lg">{t('studio.share_shop')}</h3>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            <button onClick={() => handleShare('whatsapp')} className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-xs font-bold text-gray-600">{t('studio.share_wa')}</span>
            </button>
            ...
            <button onClick={() => handleShare('facebook')} className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-xs font-bold text-gray-600">{t('studio.share_fb')}</span>
            </button>

             <button onClick={() => handleShare('telegram')} className="flex flex-col items-center gap-2 group">
              <div className="w-14 h-14 bg-sky-500 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-sky-200 group-hover:scale-110 transition-transform">
                <Send size={28} />
              </div>
              <span className="text-xs font-bold text-gray-600">{t('studio.share_tg')}</span>
            </button>
            ...
            <button onClick={() => handleShare('linkedin')} className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-xs font-bold text-gray-600">{t('studio.share_li')}</span>
            </button>
            ...
            <button onClick={() => handleShare('twitter')} className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-xs font-bold text-gray-600">{t('studio.share_tw')}</span>
            </button>
          </div>
          ...
          <div className="relative group">
            <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl p-4 transition-colors group-hover:border-brand-blue">
              ...
              <div className="flex-1 min-w-0">
                 <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">{t('studio.copy_link')}</p>
                 <input 
                    readOnly 
                    value={url} 
